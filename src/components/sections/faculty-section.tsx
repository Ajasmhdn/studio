...
const facultyData: FacultyMember[] = [
  { 
    ...
    description: 'Dr. Imthias Ahamed T. P has 25 years of teaching and research experience, including positions in Kind Saud University, Riyadh, and Dhofar University, Oman. He is currently Professor at Centre for AI, TKMCE. His research includes reinforcement learning, demand response, neural networks, and power system scheduling and control.',
  ...
  { 
    ...
    description: 'Dr. Sumod Sundar is Associate Professor at Centre for AI, TKMCE. He has trained over 3000 professionals and students, acted as resource person in 65+ sessions, and contributed to workshops and international conferences.',
  ...
  { 
    ...
    description: 'Dr. Chinnu Jacob has nearly 12 years of teaching experience. She recently submitted her PhD on "Detection, Classification, and Staging of Lung Cancer Using Deep Learning Techniques".',
  ...
  { 
    ...
    description: 'Dr. Neena Aloysius received her Ph.D. from Amrita Vishwa Vidyapeetham funded by MeitY. She has over nine years of research and industry experience including roles as Deep Learning Engineer and Senior Systems Engineer at Infosys.',
  ...
  { 
    ...
    designation: 'Honorary Professor, Dept of CSE - Research', 
...
const regularFaculty = facultyData.filter(f => f.category === 'Regular');
const honoraryFaculty = facultyData.filter(f => f.category === 'Honorary');
...
const FacultyCard = ({ member }: { member: FacultyMember }) => (
  <Card className="flex flex-col overflow-hidden transition-all hover:shadow-xl hover:-translate-y-1 duration-300 h-full">
      ...
      {member.areasOfInterest?.length > 0 && (
        <div className="mt-4">
          <h4 className="text-sm font-semibold text-foreground mb-2 flex items-center gap-2">
            <Microscope className="h-4 w-4 text-primary" /> Areas of Interest
          </h4>
    ...
    <CardFooter className="p-4 pt-0 flex flex-wrap gap-2 justify-center bg-muted/50">
      ...
      {member.email && member.email !== '#' && <Button asChild size="sm" variant="outline"><a href={`mailto:${member.email}`}><Mail /> Email</a></Button>}
    ...
    <>
      ...
      <section id="faculty" className="py-16 lg:py-24">
        <div className="container mx-auto max-w-7xl px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {regularFaculty.map(member => <FacultyCard key={member.name} member={member} />)}
          </div>
        </div>
      </section>

      {/* Honorary Faculty Section */}
      <section id="honorary-faculty" className="py-16 lg:py-24 bg-secondary">
        <div className="container mx-auto max-w-7xl px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:max-w-xl mx-auto gap-8">
            {honoraryFaculty.map(member => <FacultyCard key={member.name} member={member} />)}
          </div>
        </div>
      </section>
    </>
